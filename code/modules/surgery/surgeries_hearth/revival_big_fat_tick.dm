...
/datum/surgery_step/infuse_tick/success(mob/user, mob/living/target, target_zone, obj/item/tool, datum/intent/intent)
	...
	//addtimer(CALLBACK(src, PROC_REF(deathmark), target), 5 MINUTES) //Performs a check after the listed time has elapsed, post-resurrection. If the target is still alive by then, it'll apply the 'DNR' trait. //OV Edit - Commented Out
	return TRUE

/*/datum/surgery_step/infuse_tick/proc/deathmark(mob/living/victim) //OV Edit - Commented Out
	if(victim.stat != DEAD)
