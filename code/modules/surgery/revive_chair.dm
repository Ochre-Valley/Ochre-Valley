...
/obj/structure/chair/frankenstein/MiddleClick(mob/user)
	...
	if(occupant.revive(full_heal = FALSE))
		...
		//addtimer(CALLBACK(src, PROC_REF(deathmark), occupant), 5 MINUTES) //Performs a check after the listed time has elapsed, post-resurrection. If the target is still alive by then, it'll apply the 'DNR' trait. //OV Edit - Commented Out
		return TRUE
