...
/atom/movable/proc/send_speech_emote(message, range = 7, obj/source = src, bubble_type, list/spans, datum/language/message_language = null, message_mode, original_message)
	...
	var/list/listening = get_hearers_in_view(range, source) //OV Edit - Replaced hearers with listening
	add_remote_hearing_atom_listeners(listening, source, range) //OV Add
	for(var/_AM in listening) //OV Edit - Replaced hearers with listening
		var/atom/movable/AM = _AM
		AM.Hear(rendered, src, message_language, message, , spans, message_mode)

	if(SEND_SIGNAL(src, COMSIG_MOVABLE_QUEUE_BARK, listening, args) || vocal_bark || vocal_bark_id) //OV Edit - Replaced hearers with listening
		for(var/mob/M in listening) //OV Edit - Replaced hearers with listening
			if(!M.client)
				continue
			if((M.client.prefs.mute_barks))
				listening -= M //OV Edit - Replaced hearers with listening
		...
		for(var/i in 1 to barks)
			if(total_delay > BARK_MAX_TIME)
				break
			addtimer(CALLBACK(src, PROC_REF(bark), listening, range, vocal_volume, BARK_DO_VARY(vocal_pitch, vocal_pitch_range), vocal_current_bark), total_delay) //OV Edit - Replaced hearers with listening
