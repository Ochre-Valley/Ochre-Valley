...
/mob/living/carbon/human/proc/make_sucking_noise(do_subtle = FALSE)
	if(gender == FEMALE)
		playsound(src, pick('sound/misc/mat/girlmouth (1).ogg','sound/misc/mat/girlmouth (2).ogg'), 25, TRUE, (do_subtle ? -6 : 0), ignore_walls = FALSE, quiet = TRUE) //OV EDIT
	else
		playsound(src, pick('sound/misc/mat/guymouth (2).ogg','sound/misc/mat/guymouth (3).ogg','sound/misc/mat/guymouth (4).ogg','sound/misc/mat/guymouth (5).ogg'), 35, TRUE, (do_subtle ? -6 : 0), ignore_walls = FALSE, quiet = TRUE) //OV EDIT
